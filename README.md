...
33개. **규격 검증의 경계값과 실패 경로**를 주로 시험합니다.

| 무엇을 | 왜 |
| --- | --- |
| 280자 **정확히는 통과**, 281자부터 위반 | 경계에서 한 칸 어긋나면 멀쩡한 글이 버려집니다 |
| 한 플랫폼이 실패해도 나머지 진행 | 여섯 조합 중 하나가 이상해도 다섯은 나와야 합니다 |
| 규격 위반을 **버리지 않고 표시만** | 사람이 보고 판단할 문제입니다 |
| 이미지가 막혀도 텍스트는 저장 | 이 워크플로우의 약속입니다 |
| 프롬프트에 브랜드가 들어가는지 | 안 넣으면 브랜드 톤이 안 나옵니다 |

실제 API 는 부르지 않습니다. 키가 없어도, CI 에서도 그대로 돌아갑니다.

> 제출 전 코드 리뷰에서 결함 8건을 찾아 고쳤습니다.
> 그중 6건은 A2-1·A2-2·A2-3 과 **같은 AI 클라이언트 구조를 쓰다 따라온 것**입니다.
> 회귀 테스트를 수정 전 소스에 대고 돌려 9개가 실패하는 것을 확인했습니다.
