...
tests/                      규격 검증·재시도·저장 단위 테스트
samples/                    실제로 돌린 결과물
CLAUDE.md                   개발 방식(랄프 루프) 규칙
PROMPT.md                   루프가 매 반복마다 읽는 프롬프트
ralph.sh                    랄프 루프 실행기
fix_plan.md                 루프의 백로그(= 기억 장치)
```
...
## 개발 방식 — 랄프 루프 (Ralph Loop)

이 저장소는 **랄프 루프**로 개발합니다. Geoffrey Huntley 가 제안한 방식으로,
같은 프롬프트를 에이전트에게 반복해서 먹이는 단순한 구조입니다.

```bash
./ralph.sh        # 무한 반복 (Ctrl+C 로 중단)
./ralph.sh 5      # 5회만
```

스크립트가 하는 일은 사실상 이 한 줄입니다.

```bash
while :; do cat PROMPT.md | claude -p ; done
```

**핵심은 반복마다 컨텍스트를 버린다는 점입니다.** 대화가 길어지며 품질이
떨어지는 문제를 피하는 대신, 기억을 대화가 아니라 **파일과 git 에** 남깁니다.

| 파일 | 역할 |
| --- | --- |
| [`CLAUDE.md`](CLAUDE.md) | 매 세션이 자동으로 읽는 작업 규칙 |
| [`PROMPT.md`](PROMPT.md) | 루프가 반복마다 먹이는 프롬프트 |
| [`fix_plan.md`](fix_plan.md) | 백로그 — 루프의 기억 장치 |
| `ralph.sh` | 실행기 |

한 반복에서는 `fix_plan.md` 의 **가장 중요한 항목 딱 하나**만 골라 끝내고,
검증하고, 백로그를 갱신하고, 커밋합니다. 범위를 넓히지 않는 것이 규칙입니다.
루프가 돌수록 백로그가 줄어듭니다.

```bash
python -m unittest discover tests    # 매 반복의 검증 수단
```

지금까지 이 방식으로 규격 위반 재시도, 단위 테스트 14건,
같은 주제 재실행 시 결과 보존을 한 반복에 하나씩 붙였습니다.
